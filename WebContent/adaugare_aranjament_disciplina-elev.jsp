...
<head>
    ...
    <title>Adaugare Disciplina - Elev</title>
...
<%
    ...
    for(int j = 0; j< iddiscipline.length; j++)
      jb.adaugaAranjament(Integer.parseInt(iddiscipline[j]), value);
    jb.disconnect();
	%>
  <div class="container-fluid col-md-12 margins">
	  <div class="page-header margins">
	    <h1>Adaugare reusita</h1>
	  </div>
	  <div class="col-md-6 margins">
      <a class="btn btn-primary" href="vizualizare_elev.jsp?id=<%=idelev%>">Inapoi la elev</a>
    </div>
	</div>
	<% }
	else{
...
<div class="container-fluid col-md-12 row">
  ...
  rs = jb.getTheRestDisciplineForIdElev(idElev);
  if(rs!=null){
  %>
  <div class="col-md-2">
  <form class="form-group" action="adaugare_aranjament_disciplina-elev.jsp" method="post">
     <select multiple class="selectpicker" NAME="iddisciplina">
     <%
       while(rs.next()){
    ...
    %>
     ...
     <div style="margin-top: 8px;"><input class="btn btn-primary" type="submit" value="Adauga Discipline" /></div>
  </form>
  </div>
  <% } else { %>
  <div class="col-md-4">
      <h4> Elevul este inrolat la toate disciplinele</h4>
